...
class HomeLinkSpider(object):
    ...
    def get_max_page(self):
        response = httpx.get(self.url, headers={"User-Agent": UserAgent(use_cache_server=False).random})
    ...
    def parse_single_page(self, url):
        ...
        response = httpx.get(url, headers={"User-Agent": UserAgent(use_cache_server=False).random})
        selector = Selector(response.text)
